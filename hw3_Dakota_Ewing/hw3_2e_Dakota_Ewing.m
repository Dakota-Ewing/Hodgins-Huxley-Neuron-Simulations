...
dVdt = @(n,m,h,vt,gk,gna,gnak,gl,ca)(ca +gnak*Ip -gk*Ik(n,vt) -gna*INa(m,h,vt) -gl*Il(vt))/Cm;
...
% on.
NaKg = 1;
% The gate for the leakage pump. when 0, the Na-K pump is off, when 1, it 
% is on.
Lg = 1;
...
Vk = 50;
% The leakage pump deactivation threshold.
Vl = Leak_eq_dis;
...
for s = 1:Steps
    ...
    v1 = dVdt(N(s), M(s), H(s), V(s), Kg, Nag, NaKg, Lg, Ia)*dt;
    ...
    v2 = dVdt(N(s)+n1/2, M(s)+m1/2, H(s)+h1/2, V(s)+v1/2, Kg, Nag, NaKg, Lg, Ia)*dt;
    ...
    v3 = dVdt(N(s)+n2/2, M(s)+m2/2, H(s)+h2/2, V(s)+v2/2, Kg, Nag, NaKg, Lg, Ia)*dt;
    ...
    v4 = dVdt(N(s)+n3, M(s)+m3, H(s)+h3, V(s)+v3, Kg, Nag, NaKg, Lg, Ia)*dt;
    ...
    %Leakage Pump switch. If the sodium ion concentrain inside the membrane
    %and the potassium ion concentration outside the membrane are greater
    %than zero, the Na-K switch is on, else it is off.
    if  V(s)<=Vl
        Lg = 1;
...
title('2.e Graph of Membrane Potential Over Time:')
xlabel('Time(ms)') 
ylabel('Voltage(mV)') 
figure;
...
title('2.e Graph of n, m and h Over Time:')
xlabel('Time(ms)') 
ylabel('Probability(%)') 
